...
export class BaseCarReqDto {
  ...
  @Transform(TransformHelper.trim)
  @Type(() => String)
  @ApiProperty({ example: 'Installation of additional aluminum engine protection.'})
  description: string;

  @IsNumber({ maxDecimalPlaces: 2 }, { message: 'Entered Price must be a number with up to 2 decimal places' })
  ...
  @IsEnum(ECurrency, { message: `Entered Currency must be one of the following values: ${Object.values(ECurrency)}` })
  ...
  @Max(TimeHelper.getCurrentYear(), { message: `Year must be less than or equal to ${TimeHelper.getCurrentYear()}` })
